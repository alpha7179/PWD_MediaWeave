...
│  │  - Users, Courses, Projects, Tracks                 │   │
│  │  - TeamPosts, Messages, Transcripts                 │   │
│  │  - SavedCurriculums, Comments, Likes                │   │
│  │  - Notifications, ProjectViews                      │   │
│  └─────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
                            ↕
┌─────────────────────────────────────────────────────────────┐
...
- **Database:** Single Database - PostgreSQL 단일 데이터베이스 (MongoDB 미사용)
...
| **Tesseract.js** | 7.x | OCR 기능 (성적표 이미지 인식) |
| **@dnd-kit** | 6.x | 드래그앤드롭 기능 |
...
│   ├── common/          # 공통 컴포넌트
│   │   ├── Portal.jsx   # 모달 포털
│   │   ├── AnimatedEmoji.jsx # 애니메이션 이모지
│   │   ├── CourseDetailModal.jsx # 과목 상세 모달
│   │   └── TrackSelect.jsx # 트랙 선택 컴포넌트
│   ├── layout/          # 레이아웃 컴포넌트
│   │   ├── Layout.jsx   # 메인 레이아웃
│   │   ├── Header.jsx   # 헤더
│   │   ├── Sidebar.jsx  # 사이드바
...
│   └── SettingsPage.jsx
├── hooks/               # 커스텀 훅
│   └── useDragAndDrop.js # 드래그앤드롭 훅
├── services/            # API 호출 함수
│   ├── api.js           # Axios 인스턴스 설정
│   └── curriculumLoader.js # 커리큘럼 데이터 로더
├── stores/              # Zustand 스토어
│   ├── authStore.js     # 인증 상태
│   ├── curriculumStore.js # 커리큘럼 상태
│   ├── profileStore.js  # 프로필 상태
│   └── uiStore.js       # UI 상태 (다크모드, 모달)
├── utils/               # 유틸리티 함수
│   ├── avatarUtils.js   # 아바타 생성
│   ├── trackColors.js   # 트랙 색상 관리
│   ├── numberFormat.js  # 숫자 포맷팅
│   ├── templateBasedOCR.js # OCR 템플릿 처리
│   ├── imagePreprocessing.js # 이미지 전처리
│   └── transcriptTemplate.js # 성적표 템플릿
├── App.jsx              # 루트 컴포넌트
├── main.jsx             # 엔트리 포인트
...
**2. 커리큘럼 스토어 (curriculumStore.js)**
```javascript
const useCurriculumStore = create(
  persist(
    (set, get) => ({
      semesters: initialSemesters, // 1-1부터 4-2까지
      availableCourses: [],
      tracks: [],
      prerequisites: [],
      loadedCurriculum: null,

      moveCourse: (courseId, fromContainer, toContainer, newIndex) => {
        // 드래그앤드롭으로 과목 이동
      },
      ...
      loadSavedCurriculum: (curriculum) => {
        // 저장된 커리큘럼 로드
      },
    }),
...
| **PostgreSQL** | 15.x | 관계형 데이터베이스 (단일 DB) |
...
│   │   ├── Project.js       # 프로젝트 모델
│   │   ├── Track.js         # 트랙 모델
│   │   ├── TeamPost.js      # 팀 빌딩 게시글
│   │   ├── Message.js       # 채팅 메시지
│   │   ├── Transcript.js    # 성적표 데이터
│   │   ├── SavedCurriculum.js # 저장된 커리큘럼
│   │   ├── Comment.js       # 댓글
│   │   ├── Like.js          # 좋아요
│   │   └── Notification.js  # 알림
│   ├── controllers/         # 비즈니스 로직
...
#### 3.4.2. 프로젝트 API (`/api/v1/projects`)

```javascript
GET    /api/v1/projects          // 프로젝트 목록 조회
POST   /api/v1/projects          // 프로젝트 생성 (인증 필요, 파일 업로드)
PUT    /api/v1/projects/:id      // 프로젝트 수정 (인증 필요)
DELETE /api/v1/projects/:id      // 프로젝트 삭제 (인증 필요)

// 커뮤니티 기능
POST   /api/v1/projects/:id/like     // 좋아요 토글 (인증 필요)
POST   /api/v1/projects/:id/view     // 조회수 증가 (인증 필요)
GET    /api/v1/projects/:id/comments // 댓글 목록
POST   /api/v1/projects/:id/comments // 댓글 작성 (인증 필요)
PUT    /api/v1/projects/comments/:commentId    // 댓글 수정 (인증 필요)
DELETE /api/v1/projects/comments/:commentId    // 댓글 삭제 (인증 필요)
```

#### 3.4.3. 팀 빌딩 API (`/api/v1/teams`)

```javascript
GET    /api/v1/teams             // 팀 빌딩 게시글 목록
POST   /api/v1/teams             // 팀 빌딩 게시글 작성 (인증 필요)
PUT    /api/v1/teams/:id         // 게시글 수정 (인증 필요)
DELETE /api/v1/teams/:id         // 게시글 삭제 (인증 필요)

POST   /api/v1/teams/:id/apply   // 팀 지원 (인증 필요)
GET    /api/v1/teams/:id/applications // 지원자 목록 (인증 필요)
PUT    /api/v1/teams/applications/:id // 지원 상태 변경 (인증 필요)
```

#### 3.4.4. 커리큘럼 API (`/api/v1/curriculums`, `/api/v1`)
...
// 개인 플래너
GET    /api/v1/my-plan           // 개인 커리큘럼 플랜 조회 (인증 필요)
POST   /api/v1/my-plan           // 개인 커리큘럼 플랜 저장 (인증 필요)
GET    /api/v1/graduation-status // 졸업 요건 확인 (인증 필요)
```

#### 3.4.5. 성적표 API (`/api/v1/transcripts`)

```javascript
GET    /api/v1/transcripts       // 성적표 목록 (인증 필요)
POST   /api/v1/transcripts       // 성적표 업로드 (인증 필요, 파일 업로드)
DELETE /api/v1/transcripts/:id   // 성적표 삭제 (인증 필요)
```

#### 3.4.6. 메시지 API (`/api/v1/messages`)

```javascript
GET    /api/v1/messages          // 메시지 목록 (인증 필요)
POST   /api/v1/messages          // 메시지 전송 (인증 필요)
PUT    /api/v1/messages/:id/read // 메시지 읽음 처리 (인증 필요)
```

#### 3.4.7. 관리자 API (`/api/v1/admin`)

```javascript
GET    /api/v1/admin/users       // 사용자 관리 (관리자 권한)
GET    /api/v1/admin/projects    // 프로젝트 관리 (관리자 권한)
GET    /api/v1/admin/stats       // 통계 조회 (관리자 권한)
```
...
PUT    /api/v1/users/profile     // 프로필 수정 (인증 필요)

GET    /api/v1/notifications     // 알림 목록 (인증 필요)
PUT    /api/v1/notifications/:id/read // 알림 읽음 처리 (인증 필요)
...
#### 3.5.1. OCR 성적표 인식 시스템

```javascript
// Tesseract.js를 활용한 클라이언트 사이드 OCR
...
const processTranscriptImage = async (imageFile) => {
  ...
  // 템플릿 기반 데이터 추출
  const extractedData = parseTranscriptText(text);
...
1. 회원가입/로그인
   ...
   Server → Bcrypt 비밀번호 검증 (Salt Rounds: 10)
   Server → JWT 토큰 생성 (Access Token)
   Server → Response { user, accessToken }

2. 인증이 필요한 API 호출
   Client → GET /api/v1/projects (Header: Authorization: Bearer <token>)
   Server → JWT 미들웨어 검증 (middlewares/auth.js)
   Server → req.user에 사용자 정보 주입
   Controller → 비즈니스 로직 실행

3. 로그아웃
   Client → localStorage에서 토큰 제거
   Client → Zustand 스토어 초기화
```
...
### 4.1. PostgreSQL (단일 데이터베이스)

#### 주요 테이블 및 관계

```sql
-- 사용자 관리
Users (user_id, email, name, student_id, password_hash, role, bio, github_url, linkedin_url, profile_image, is_active, created_at, updated_at)

-- 학사 정보
Tracks (track_id, initials, name_ko, name_en, description, color, created_at, updated_at)
Courses (course_id, code, name_ko, name_en, credit, year, semester, credit_type, description, prerequisites, created_at, updated_at)

-- 관계 테이블
course_tracks (course_id, track_id) -- Course와 Track의 N:M 관계

-- 커뮤니티
Projects (project_id, user_id, track_id, course_id, title, description, thumbnail, project_type, start_date, end_date, tech_stacks, views, likes, created_at, updated_at)
Comments (id, user_id, project_id, content, created_at, updated_at)
Likes (id, user_id, project_id, created_at, updated_at)
ProjectViews (id, user_id, project_id, created_at, updated_at)
project_pins (user_id, project_id) -- 사용자가 핀한 프로젝트

-- 팀 빌딩
TeamPosts (id, author_id, title, description, track_code, tags, max_members, status, recruitment_deadline, project_period, created_at, updated_at)
TeamApplications (id, post_id, applicant_id, message, status, created_at, updated_at)

-- 채팅 시스템
Messages (id, sender_id, receiver_id, team_post_id, project_id, profile_id, content, is_read, created_at, updated_at)

-- 성적표 및 커리큘럼
Transcripts (transcript_id, user_id, semester, course_code, course_name, credit, grade, grade_point, course_type, created_at, updated_at)
SavedCurriculums (id, user_id, name, description, track, curriculum_data, is_shared, shared_by, origin_id, views, copies, created_at, updated_at)
SavedCurriculumViews (id, user_id, curriculum_id, created_at, updated_at)
StudentCurriculums (id, user_id, course_id, plan_semester, status, created_at, updated_at)

-- 알림 시스템
Notifications (id, user_id, type, title, message, is_read, related_id, created_at, updated_at)
```

#### 핵심 관계 정의

```javascript
// models/index.js에서 정의된 실제 관계

// 1. Course ↔ Track (N:M)
Course.belongsToMany(Track, { through: 'course_tracks', foreignKey: 'course_id' });
Track.belongsToMany(Course, { through: 'course_tracks', foreignKey: 'track_id' });

// 2. Project 관계
Project.belongsTo(Track, { foreignKey: 'track_id' });
Project.belongsTo(User, { foreignKey: 'user_id' });
Project.belongsTo(Course, { foreignKey: 'course_id' });

// 3. 커뮤니티 기능
User.belongsToMany(Project, { through: Like, foreignKey: 'user_id', as: 'LikedProjects' });
Project.belongsToMany(User, { through: Like, foreignKey: 'project_id', as: 'Likers' });
Project.hasMany(Comment, { foreignKey: 'project_id' });
Project.hasMany(ProjectView, { foreignKey: 'project_id' });

// 4. 팀 빌딩
User.hasMany(TeamPost, { foreignKey: 'author_id' });
TeamPost.hasMany(TeamApplication, { foreignKey: 'post_id' });
User.hasMany(TeamApplication, { foreignKey: 'applicant_id' });
...
// 6. 커리큘럼 관리
User.belongsToMany(Course, { through: StudentCurriculum, foreignKey: 'user_id' });
User.hasMany(SavedCurriculum, { foreignKey: 'user_id' });
User.hasMany(Transcript, { foreignKey: 'user_id' });
```
...
CREATE INDEX idx_projects_user_track ON projects(user_id, track_id);
CREATE INDEX idx_projects_created ON projects(created_at DESC);
CREATE INDEX idx_messages_sender_receiver ON messages(sender_id, receiver_id);
CREATE INDEX idx_transcripts_user_semester ON transcripts(user_id, semester);
...
PostgreSQL의 JSON 타입을 활용하여 유연한 데이터 구조 저장:

```sql
-- 커리큘럼 데이터 (JSON 형태로 저장)
SavedCurriculums.curriculum_data: {
  "1-1": [
    {
      "id": "CSE200",
      "name": "프로그래밍 기초",
...
-- 프로젝트 기술 스택 (문자열로 저장)
Projects.tech_stacks: "React,Node.js,PostgreSQL"

-- 팀 빌딩 태그 (문자열로 저장)
TeamPosts.tags: "Unity,React,AI"
```
...
```sql
-- 파일 경로만 데이터베이스에 저장
Users.profile_image: "/uploads/profiles/user123.jpg"
Projects.thumbnail: "/uploads/projects/project456.jpg"

-- 실제 파일은 서버 로컬 디스크에 저장
server/uploads/
├── profiles/
├── projects/
└── transcripts/
...
**MongoDB를 사용하지 않은 이유:**
- 프로젝트 규모가 작아 NoSQL의 확장성 이점이 불필요
- 관계형 데이터가 대부분이어서 PostgreSQL이 더 적합
- 팀의 PostgreSQL 경험이 MongoDB보다 풍부
- 인프라 복잡성 최소화 (단일 DB 서버 운영)
...
| **PostgreSQL** | MySQL, MongoDB | 복잡한 관계 처리, JSON 타입 지원, ACID 트랜잭션 |
| **Sequelize** | Prisma, TypeORM | 성숙한 ORM, PostgreSQL 완벽 지원 |
| **Express** | Nest.js, Fastify | 간단한 구조, 빠른 개발, 풍부한 미들웨어 |
| **Zustand** | Redux, Context API | 간단한 API, 보일러플레이트 최소화 |
| **React Query** | SWR, Apollo Client | 강력한 캐싱, 서버 상태 관리 특화 |
| **Tesseract.js** | Google Vision API | 무료, 클라이언트 사이드 처리 |
| **@dnd-kit** | React DnD | 접근성 지원, 터치 디바이스 호환 |
| **Tailwind CSS** | Styled Components, CSS Modules | 유틸리티 우선, 빠른 스타일링 |
| **JWT** | Session | Stateless, 확장성 우수 |
| **Multer** | AWS S3 Direct Upload | 간단한 구현, 로컬 파일 저장 |

### 주요 설계 결정

#### 1. MongoDB 제외 결정
- **초기 계획:** PostgreSQL + MongoDB 하이브리드
- **최종 결정:** PostgreSQL 단일 데이터베이스
- **이유:** 
  - 프로젝트 규모가 작아 NoSQL 이점 불필요
  - PostgreSQL JSON 타입으로 유연성 확보
  - 운영 복잡성 최소화
  - 팀의 PostgreSQL 경험 활용

#### 2. 실시간 기능 구현
- **채팅 시스템:** 폴링 방식으로 구현 (WebSocket 대신)
- **알림 시스템:** 데이터베이스 기반 알림
- **이유:** 구현 복잡성 최소화, MVP 요구사항 충족

#### 3. 파일 저장 전략
- **선택:** 로컬 파일 시스템 저장
- **대안:** AWS S3 직접 업로드
- **이유:** 개발 단순성, 비용 절약, 빠른 구현

#### 4. OCR 기능 추가
- **추가된 기능:** Tesseract.js를 활용한 성적표 OCR
- **이유:** 사용자 편의성 향상, 데이터 입력 자동화
- **구현:** 클라이언트 사이드 처리로 서버 부하 최소화
